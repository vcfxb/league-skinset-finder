...
use super::generated::{ALL_SKINSET_NAMES, CHAMPS_TO_SKINSETS, LANE_DATA};
...
impl ChampId {
    ...
    pub fn skinsets(
        self,
    ) -> impl DoubleEndedIterator<Item = SkinsetId> + ExactSizeIterator + FusedIterator {
        CHAMPS_TO_SKINSETS[self.0]
            .iter()
            .map(|index: &usize| SkinsetId(*index))
    }
}
